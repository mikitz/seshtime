...
function calculateTTL(sessionDate, sessionTime, sessionTimezone) {
    ...
    if (sessionDate.includes("-")) sessionDateSplit = sessionDate.split("-");
        ...
        sessionDateSplit = sessionDate.split("/");

    let hourOffset = 0;
    if (sessionTime.includes("pm")) hourOffset = 12;
    const sessionHour =
        parseInt(sessionTime.replace("pm", "").replace("am", "")) + hourOffset;
    ...
    const sessionTTL = Math.abs(
        sessionDateTime.toMillis() - dateTimeNow.toMillis()
    );
    return {
...
async function determineEventStatus(guildId, eventObject, user, buttonId) {
    ...
    if (buttonId === "not-attending") {
        ...
        // Insufficient Players
            ...
            notAttending > groupSize - minPlayers
        ) {
...
async function getMembersByRole(guildId, roleId, client, authorId) {
    ...
    // console.log("🚀 ~ getMembersByRole ~ guild:", guild);
    if (!guild) return [];
    let members;
    try {
...
async function sendDirectMessage(client, userId, messageText) {
    try {
        ...
        logger.error(
            `Could not send DM to user with ID ${userId}. Error: ${error}`
        );
        return "error";
    }
}
async function sendMessageToChannel(client, channelId, messageText) {
    try {
        ...
        logger.error(
            `Could not send message to channel ID ${channelId}. Error: ${error}`
        );
    }
}
