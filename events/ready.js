			...
			for (let index = 0; index < guilds.length; index++) {
				...
				for (let index = 0; index < events.length; index++) {
					...
					if (now >= rsvpDeadline || rsvpDeadline == undefined) {
						console.log(`[${now.toLocaleString(DateTime.DATETIME_SHORT_WITH_SECONDS)}] ------ CHECKING STATUS -- Guild ${guild} : Status ${status}`)
						if (status && status.includes('canceled')) continue
						console.log(`[${now.toLocaleString(DateTime.DATETIME_SHORT_WITH_SECONDS)}] ----- CANCELING EVENT -- Guild ${guild} : Event ${event.messageId}`)
						// Cancel Event
