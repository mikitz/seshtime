...
async function addEvent(guildId, eventObject) {
    ...
    logger.log(
        `CREATED -- Guild ${guildId} : Event ${eventObject.messageId} added successfully!`
    );
}
async function updateEvent(
    ...
    await keyv.set("events", JSON.stringify(events));
    logger.log(
        `UPDATED -- Guild ${guildId} : Event ${messageId} updated successfully! ${memberNickname} is now ${attendanceStatus}.`
    );
}
async function deleteExpiredEvents(guildId) {
    ...
    events = JSON.parse(events);
    logger.log(
        `DELETING -- Guild ${guildId} : Scanning ${events.length} events for possible deletion...`
    );
    let eventsToKeep = [];
    console.log("🚀 ~ deleteExpiredEvents ~ events:", events);

    for (let i = 0; i < events.length; i++) {
        const event = events[i];
        const datetime = DateTime.fromISO(event.datetime);
        const now = DateTime.now();
        if (datetime > now) eventsToKeep.unshift(event);
    }
    await keyv.set("events", JSON.stringify(eventsToKeep));
    logger.log(
        `DELETING -- Guild ${guildId} : Deleted ${
            events.length - eventsToKeep.length
        } event(s)`
    );
}
